...
from .controllers import order_detials, orders, sandwiches, recipes, resources
...
@app.post('/order_detial/',response_model=schemas.OrderDetail, tags=["OrderDetails"])
def create_order_detail(order_detail:schemas.OrderDetailCreate, db:Session = Depends(get_db)):
    return order_detials.create(db=db, order_detail=order_detail)


@app.get("/order_detial/", response_model=list[schemas.OrderDetail], tags=["OrderDetails"])
def read_order_detail(db: Session = Depends(get_db)):
    return order_detials.read_all(db)



@app.get("/order_detial/{order_detail_id}", response_model=schemas.OrderDetail, tags=["OrderDetails"])
def read_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    order_detail = order_detials.read_one(db, order_detail_id=order_detail_id)
    if order_detail is None:
        raise HTTPException(status_code=404, detail="Order Detail not found")
    return order_detail


@app.put("/order_detial/{order_detail_id}", response_model=schemas.OrderDetail, tags=["OrderDetails"])
def update_one_order_detail(order_detail_id: int, order_details_data: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    order__details_db = order_detials.read_one(db,  order_detail_id=order_detail_id)
    if order__details_db is None:
        raise HTTPException(status_code=404, detail="User not found")
    return order_detials.update(db=db, order_detail=order_details_data, order_detail_id=order_detail_id)


@app.delete("/order_detial/{order_detail_id}", tags=["OrderDetails"])
def delete_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    order_detail = order_detials.read_one(db, order_detail_id=order_detail_id)
    if order_detail  is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
    return order_detials.delete(db=db, order_detail_id=order_detail_id)
...
@app.get("/recipe/", response_model=list[schemas.Recipe], tags=["Recipes"])
...
@app.get("/resource/", response_model=list[schemas.Resource], tags=["Resources"])
...
@app.put("/resource/{resource_id}", response_model=schemas.Resource, tags=["Resources"])
